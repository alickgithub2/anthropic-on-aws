...
export interface GatewayStackProps extends cdk.StackProps {
  ...
  /** Route 53 PRIVATE hosted-zone name, e.g. example.com (ECS, holds the A-record). */
  readonly zoneName?: string;
  /** Route 53 private hosted-zone id (optional; looked up from zoneName if omitted). */
  readonly zoneId?: string;
  /** PUBLIC hosted-zone id — MANAGED cert mode only (ECS); used solely for ACM DNS validation. */
  readonly publicZoneId?: string;
  /** PUBLIC hosted-zone name — MANAGED cert mode only (ECS); explicit, not derived from zoneName. */
  readonly publicZoneName?: string;
  /** Deploy the CloudWatch dashboard + alarms (ECS, default false). */
  readonly enableDashboard?: boolean;
  /** Daily cost-alarm threshold in USD (ECS dashboard mode; enables the cost alarm when > 0). */
  readonly dailyCostThresholdUsd?: number;
  /** Optional email for an SNS alarm subscription (ECS dashboard mode). */
 ...
 *
 * The ECS path additionally supports the managed-public-cert mode and the
 * CloudWatch dashboard + alarms preserved from PR #233; the EKS path serves the
 * same image behind an Auto Mode internal ALB Ingress.
 */
